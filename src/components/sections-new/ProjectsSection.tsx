...
import PurpleThing from "../../images/purple-thing.svg"
...
const projects = [
  {
    ...
    accentColor: 'violet',
    summary: <>The idea behind SmartNotes is to provide a tool transforming ideas into composed text to boost productivity and even creativity. By processing any contextual knowledge useful for the desired output, SmartNotes aims to become the next iteration of text editors.</>,
    details: <>Using various NLP models from HuggingFace, GPT-3 and knowledge graphs, SmartNotes understands what you want to write, identify the relevant information and processes it to proide various functionalities such as next phrase autocompletion, notes-to-text, question generation, etc.</>,
  ...
  },
  {
    ...
    linkTags: [
      <>Learn more about it in the <Link href="#" color="teal-darker">About</Link> section</>
    ]
  },
  {
    title: <>Ant Colony Optimization <Text as="span" color="violet">algorithm</Text></>,
    accentColor: 'violet',
    details: <>“Ant colony” is a fascinating optimization algorithm inspired by how ants find the shortest path between food sources and their colony.<br />I decided to leverage Elixir’s actor model to simulate ants in their own processes and solve the famous Travelling Salesman Problem (TSP). This approach makes the algorithm extremely scalable as it is then able to run on thousands of processors at the same time.</>,
    techStackTag: 'Python, Node.js, TypeScript, GraphQl, HuggingFace, NLTK, GPT-3',
    linkTags: [
      <>Checkout the code on <Link href="#" color="violet">Github</Link></>
    ]
    ...
    <Box pos="relative">
      <Section sectionId={SectionIds.projects} w="100%" alignItems="end">
          <Image
            ...
            left={["-100px", "-25px"]}
          />
          <Text color="black" as="h2" textStyle="h2">Projects</Text>
...
const ProjectShowcase: React.FC<{ project: Project }> = ({ project }) => {
  return (
    <Box>
      <Text as="h3" textStyle="h3" >{project.title}</Text>
      ...
      <Text as="p" textStyle="p2" my={6} lineHeight={1.4}>{project.details}</Text>
      <Box display="flex" flexWrap="wrap" gridGap={[2, 4]}>
        <TechStackTag
          text={project.techStackTag}
          accentColor={project.accentColor}
        />
        {project.linkTags.map(tag => <LinkTag accentColor={project.accentColor}>{tag}</LinkTag>)}
      </Box>
    </Box>
  )
}
